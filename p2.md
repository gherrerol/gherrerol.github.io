...
El reto reside en obtener una nube de puntos que represente los bordes de los objetos de la escena, para ello deberemos usar los conceptos aprendidos en clase sobre retropoyección, matching de puntos y tener en cuenta las restricciones de la franja epipolar y la atentiva.
...
* **Matriz P2 (Cámara Derecha):** es idéntica a P1, pero le indicamos que está desplazada en el eje X según la línea base. Ese desplazamiento se codifica en la última columna multiplicando la focal por la línea base ($-F \cdot B$).
...
Además de calcular la recta epipolar, el proceso de emparejamiento (matching) puede sufrir de ambigüedad. Para evitar que el sistema empareje mal los puntos, implementé la restricción atentiva:

* **Restricción Direccional:** por la disposición física del hardware, la cámara derecha está trasladada en el eje X positivol, lo que significa que cualquier objeto capturado aparecerá visualmente desplazado hacia la izquierda en su plano imagen. Por tanto, el algoritmo recorta la búsqueda en la recta epipolar para iterar exclusivamente en esa dirección (asumiendo siempre una disparidad estéreo positiva).
* **Restricción de Unicidad:** en el mundo físico, un punto 3D solo puede proyectarse en un único píxel de la cámara izquierda y en un único píxel de la cámara derecha, sin embargo, matemáticamente, el *Template Matching* podría encontrar que varios píxeles izquierdos se parecen mucho a un único píxel derecho. Para forzar la unicidad a la hora de triangular, me guardo una lista con el pixel a emparejar, si ya está en la lista paso al siguiente y sino lo guardo.
Esta combinación de la estricta línea epipolar y las dos restricciones atentivas reduce los falsos positivos y emparejamientos ambiguos¡.
...
Seguramente queden por probar muchas optimizaciones, como implementar precisión subpíxel para suavizar las "capas" de profundidad, o aplicar validación cruzada (izquierda-derecha y derecha-izquierda) para limpiar el 100% de los falsos positivos, pero aún así, considero que se ha obtenido una nube de puntos lo suficientemente densa y precisa para el objetivo de la práctica.
